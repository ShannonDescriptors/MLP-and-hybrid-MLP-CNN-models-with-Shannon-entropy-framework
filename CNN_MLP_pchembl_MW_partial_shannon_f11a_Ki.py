...
from image_processing.datasets_molpred_2D_1image_resnet import image_data_extract
...
basePath = os.path.join("Ki_targets_atom_labels_wo_Milvexian_BMS") # Images with atom labels
print("basePath", basePath)

# extract out all the image paths in dataset folder
...
print("[INFO] loading images of molecules...")
...
# constructing a new df table as df_new containng only MW, shannon values ana labels
...
# split = train_test_split(images, df_new, test_size = 0.15, random_state = 42) 
...
print("Xtrain shape",XImagetrain.shape)
print("Xtest shape",XImagetest.shape)
...
cnn = KiNet_mlp.create_cnn(224, 224, 3, filters = (2,6), regress = False)

print("shape of mlp", mlp.output.shape)
print("shape of cnn", cnn.output.shape)
...
print("shape of cnn input", cnn.input.shape)


# initialize the optimizer model and compiling the model
...
plt.savefig('MLP_CNN_Ki_pred_atom_labels_wo_Milvexian_BMS.png')
plt.show()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('MLP_CNN_loss@epochs_pChEMBL_MW')
###------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
