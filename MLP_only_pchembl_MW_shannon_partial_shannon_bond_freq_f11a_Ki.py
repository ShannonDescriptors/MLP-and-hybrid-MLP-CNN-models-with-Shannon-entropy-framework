# This program predicts Ki values in the form of pCHEMBL/MW values of molecules for the target protein coagulation factor F11a with Shannon entropies, fractional Shannon entropy, bond frequency and MW as descriptors
...
# Estimating the min and max of the target column
...
# getting the total Shannon, partial or fractional shannon and bond frequency data for all molecules
fp_combined = []
fp_combined_bond = []
shannon_arr = []
...
for i in range(0,len(df_smiles)):  
  ...
  # total_shannon = df['shannon_smiles'][i] + df['shannon_inchikey'][i] + df['shannon_smarts'][i] ### Based on SMILES Shannon as total Shannon
  
  total_shannon =  df['shannon_smiles'][i] ### Based on SMILES Shannon as total Shannon
  
  ###total_shannon = df['shannon_inchikey'][i] ### Based on InChiKey Shannon as total Shannon
  ...
  fp_combined_bond.append(bond_freq)
  # for i in range(len(bond_freq)):
  #     fp_combined_bond.append(   bond_freq[i] )
...
df_new = pd.concat([ df_1, df_2, df_3, df_4, fp_mol, fp_bond, df_5 ], axis = 1)

# df_new = pd.concat([ df['full_mwt'], fp_mol, df['pChEMBL Value'] ], axis = 1)

# df_new = pd.concat([ df['full_mwt'], shannon_smiles, fp_mol, df['pChEMBL Value'] ], axis = 1)
...
# compute the difference between the predicted and actual house prices, then compute the % difference and absolute % difference
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('MLP_only_loss@epochs_pChEMBL_MW')
###------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
