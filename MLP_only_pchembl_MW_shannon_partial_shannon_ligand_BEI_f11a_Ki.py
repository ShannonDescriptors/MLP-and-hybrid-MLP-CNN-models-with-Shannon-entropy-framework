...
df_ligand_BEI = df['Ligand Efficiency BEI'].values
...
### Inserting the new column as the 1st column in df
...
#### Inserting the new column as the 3rd column in df
...
for i in range(0,len(df_smiles)):  
  ...
  # estimating partial or fractional Shannon
  ps = []
  for atom_rdkit in mol.GetAtoms():
...
df_new = pd.concat([ df_1, df_2, df['Ligand Efficiency BEI'], df_5 ], axis = 1)
...
# # FINAL MODEL taking into account numerical data from MLP 
...
epoch_number = 500 ## Larger epoch number would increase the accuracy, e.g. epoch number = 2000
BS = 5;
...
# save the network to disk
    ...
    # model.save("2D_diff.hdf5") # hdf5 access issues are there, probably due to interference from matminer/pymatgen
    # model.save("F11a_target_hybrid_model_Ki")
    # model.save("F11a_target_hybrid_model_IC50_edited_all")
    # model.save("F11a_target_hybrid_model_IC50_atom_labels")
    model.save("F11a_target_hybrid_model_Ki_atom_labels_wo_Milvexian_BMS")
